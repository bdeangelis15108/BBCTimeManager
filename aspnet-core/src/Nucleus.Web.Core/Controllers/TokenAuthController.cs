...
using Nucleus.Authentication.TwoFactor.Google;
...
using Nucleus.Web.Authentication.JwtBearer;
using Nucleus.Web.Authentication.TwoFactor;
using Nucleus.Web.Models.TokenAuth;
using Nucleus.Authorization.Impersonation;
using Nucleus.Authorization.Roles;
using Nucleus.Configuration;
using Nucleus.Debugging;
using Nucleus.Identity;
using Nucleus.Net.Sms;
using Nucleus.Notifications;
using Nucleus.Security.Recaptcha;
using Nucleus.Web.Authentication.External;
using Nucleus.Web.Common;

namespace Nucleus.Web.Controllers
{
    [Route("api/[controller]/[action]")]
    public class TokenAuthController : NucleusControllerBase
    {
        ...
        private async Task<AbpLoginResult<Tenant, User>> GetLoginResultAsync(string usernameOrEmailAddress, string password, string tenancyName, bool isMobileClient)
        {
            ...
            switch (loginResult.Result)
            {
                case AbpLoginResultType.Success:
                    ...
                    if (roles == null || roles.Count == 0)
                        throw _abpLoginResultTypeHelper.CreateExceptionForFailedLoginAttempt(AbpLoginResultType.UserIsNotActive, usernameOrEmailAddress, tenancyName, "No Roles are established for this user.");
                    else if ((roles.Contains("4f0faef04f8845ec9ab371c599c7c30b")|| roles.Contains("Payroll Admin")) && isMobileClient)
                        throw _abpLoginResultTypeHelper.CreateExceptionForFailedLoginAttempt(AbpLoginResultType.UserIsNotActive, usernameOrEmailAddress, tenancyName, "Payroll Admin cannot access Mobile Application.");
                    else if ((roles.Contains("6e9cf04eba8949b9a9c43b201cb4aae5") || roles.Contains("Shift Manager")) && !isMobileClient)
                        throw _abpLoginResultTypeHelper.CreateExceptionForFailedLoginAttempt(AbpLoginResultType.UserIsNotActive, usernameOrEmailAddress, tenancyName, "Shift Manager cannot access Web Application.");
                    return loginResult;
                default:
