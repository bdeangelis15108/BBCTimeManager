...
namespace Nucleus.ApiClient
{
    public class AbpApiClient : ISingletonDependency, IDisposable
    {
        ...
        public async Task<T> GetAsync<T>(string endpoint, object queryParameters, string accessToken, bool stripAjaxResponseWrapper)
        {
            ...
            if (stripAjaxResponseWrapper)
            {
                ...
                if (CrossConnectivity.Current.IsConnected)
                {
                    try
                    {
                        AjaxResponse<T> response = await httpResponse.GetJsonAsync<AjaxResponse<T>>();
                        ValidateAbpResponse(response);
                        Barrel.Current.Empty(httpResponse.Url);
                        Barrel.Current.Add(key: httpResponse.Url, data: response, expireIn: TimeSpan.FromDays(1));
                        return response.Result;
                    }
                    catch (Exception ex)
                    {
                ...
                {
                    var response = Barrel.Current.Get<AjaxResponse<T>>(httpResponse.Url);
                    ValidateAbpResponse(response);
                    return response.Result;
                }
        ...
        private void AddHeaders(string accessToken)
        {
            ...
            _client.WithHeader("User-Agent", NucleusConsts.AbpApiClientUserAgent);
