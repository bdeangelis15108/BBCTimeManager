...
using Nucleus.ApiClient.Models;
...
namespace Nucleus.ApiClient
{
    public class AccessTokenManager : IAccessTokenManager, ISingletonDependency
    {
        ...
        public async Task<AbpAuthenticateResultModel> LoginAsync()
        {
            ...
            using (var client = CreateApiClient())
            {
                ...
                if (CrossConnectivity.Current.IsConnected)
                {
                    ...
                    Barrel.Current.Empty("Authenticate");
                    Barrel.Current.Empty("UserLoginInfo");
                    Barrel.Current.Add(key: "Authenticate", data: response, expireIn: TimeSpan.FromDays(1));
                    Barrel.Current.Add(key: "UserLoginInfo", data: _authenticateModel, expireIn: TimeSpan.FromDays(1));
                    AuthenticateResult = response.Result;
                }
                else
                {
                    var UserLoggedInInfo = Barrel.Current.Get<AbpAuthenticateModel>("UserLoginInfo");
                    if(UserLoggedInInfo.UserNameOrEmailAddress == _authenticateModel.UserNameOrEmailAddress && UserLoggedInInfo.Password == _authenticateModel.Password)
                    {
                        var response = Barrel.Current.Get<AjaxResponse<AbpAuthenticateResultModel>>("Authenticate");
                        AuthenticateResult = response.Result;
                    }
                    else
                    {
                        AuthenticateResult = null;
                        throw new UserFriendlyException("Incorrect User name or Password");
                    }
                }
        ...
        private static IFlurlClient CreateApiClient()
        {
            ...
            client.WithHeader("User-Agent", NucleusConsts.AbpApiClientUserAgent);
            return client;
        }
    }
}