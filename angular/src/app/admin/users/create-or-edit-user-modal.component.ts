...
export class CreateOrEditUserModalComponent extends AppComponentBase {
    ...
    ngOnInit() {
        this._userService.geWorkerClaseeses().subscribe(userWorkerClaseeses => {
            console.log('userWorkerClaseeses', userWorkerClaseeses)
            this.WorkerClaseeses = userWorkerClaseeses.items
        });
    }
    ...
    OnChange(value) {
        this.EmployeeCode = this.user.surname.toUpperCase().substring(0, 4) + " " + this.user.name.toUpperCase().substring(0, 2);
    }   
    ...
    show(userId?: number): void {
        ...
        this._userService.getUserForEdit(userId).subscribe(userResult => {
            console.log('userWorkerClaseeses', userResult)
            if (!!!userResult.user) {
                userResult.user.workerClaseesId = 0;
            }
